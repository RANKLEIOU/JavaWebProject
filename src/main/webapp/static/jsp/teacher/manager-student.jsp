...
<script type="text/javascript">
	...
	function pageSelectCallback(pageIndex, jQuery) {
		...
		//跳转页面
		window.location.href = "student?page=" + pageNum + "&sno=${param.sno}" + "&sName=${param.sName}" + "&keyword=teacherInfo";
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<form action="student" method="get" class="form-inline" role="form" style="float: left;">
						<input type="hidden" name="keyword" value="teacherInfo"/>
					...
					<%--</button>--%>
					<a class="btn btn-primary" style="float: right;" href="clazz?status=isTeacherWithStudent">
							...
							<c:if test="${!empty studentList.data.data }">
								<c:forEach items="${studentList.data.data }" var="s">
									<tr>
										...
										<td>${s.getPassword()}</td>
										<td>
											<a href="student?method=toEdit&keyword=teacherPage&account=${s.getName()}"
											   class="btn btn-primary btn-xs">
												<i class=" glyphicon glyphicon-pencil"></i>
											</a>
											<span>
												<%--<form action="student" method="post" onsubmit="return del()">--%>
												<%--	<input type="hidden" name="method" value="del" />--%>
												<%--	<input type="hidden" name="id" value="${s.getId()}" />--%>
													<button id="delBtn" onclick="del(${s.getId()})" class="btn btn-danger btn-xs">
